...
function openVenueList() {
  ...
  <div class="fixed inset-0 bg-[#1C1B20]/60 backdrop-blur-sm flex items-center justify-center p-4 fade-in" onclick="handleOverlayClick(event)">
    <div class="bg-[#FCF8FF] rounded-3xl shadow-2xl w-full max-w-md scale-in overflow-hidden" onclick="event.stopPropagation()">
      <div class="bg-gradient-to-br from-[#20419A] to-[#2B54C2] p-6 text-white">
      ...
      <div class="p-4 space-y-4 max-h-[70vh] overflow-y-auto">
        ...
        <div>
          <div class="flex items-center gap-2 px-2 mb-2">
            <span class="material-symbols-outlined text-[#2B54C2] text-lg">${g.icon}</span>
            <p class="text-xs font-black text-[#787581] uppercase tracking-widest">${g.label}</p>
          </div>
          <div class="space-y-2">
            ${g.items.map(f => `
            <button onclick="openBooking('${f.id}')" class="w-full flex items-center gap-4 bg-white hover:bg-[#F7F2F9] border-2 border-transparent hover:border-[#2B54C2]/20 rounded-2xl p-4 text-left transition-all group">
              <div class="w-10 h-10 bg-[#BCCEFF] rounded-xl flex items-center justify-center flex-shrink-0">
                <span class="material-symbols-outlined text-[#20419A] text-lg">${g.icon}</span>
              </div>
              ...
              <span class="material-symbols-outlined text-[#c9c4d2] group-hover:text-[#2B54C2] transition-colors">chevron_right</span>
            </button>`).join('')}
          </div>
        </div>` : '').join('')}
      </div>
...
function openBooking(facilityId) {
  ...
  <div class="fixed inset-0 bg-[#1C1B20]/60 backdrop-blur-sm flex items-center justify-center p-4 fade-in" onclick="handleOverlayClick(event)">
    <div class="bg-[#FCF8FF] rounded-3xl shadow-2xl w-full scale-in overflow-hidden flex flex-col" style="max-width:900px;max-height:92vh" onclick="event.stopPropagation()">
      <!-- Header -->
      <div class="bg-gradient-to-br from-[#20419A] to-[#2B54C2] p-5 text-white flex-shrink-0">
      ...
      <div class="flex flex-1 overflow-hidden min-h-0">
        ...
        <div class="w-80 flex-shrink-0 p-5 space-y-4 overflow-y-auto border-r border-[#E5E1E8]">
          ...
          <div class="flex gap-3 pt-1">
            ...
            <button onclick="submitBooking('${facilityId}')" class="flex-1 py-3 rounded-full bg-gradient-to-br from-[#20419A] to-[#2B54C2] text-white font-black text-sm shadow-lg hover:brightness-110 transition-all flex items-center justify-center gap-1.5">
      ...
      <tr>
        ...
        ${weekDates.map((d, i) => {
          ...
          return `<th style="text-align:center;padding:4px 2px">
            <div style="font-size:10px;font-weight:800;color:${isSelected?'#20419A':'#787581'};text-transform:uppercase;letter-spacing:.05em">${DAY_LABELS[i]}</div>
            <div style="font-size:11px;font-weight:900;color:${isSelected?'#20419A':'#1c1b20'};background:${isSelected?'#BCCEFF':'transparent'};border-radius:6px;padding:2px 4px">${dayNum} ${mon}</div>
          </th>`;
        }).join('')}
      </tr>
    </thead>
...
function toggleBookingAmenity(id) {
  ...
  if(_bookingAmenities.has(id)) {
    ...
    btn.style.borderColor='#2B54C2'; btn.style.background='#BCCEFF'; btn.style.color='#20419A';
  }
}
...
function showToast(msg, type='info') {
  const colors={success:'bg-green-500',error:'bg-red-500',info:'bg-[#20419A]',warning:'bg-amber-500'};
...
function navigate(page) { currentPage=page; saveSessionPage(currentPage); render(); window.scrollTo(0,0); }
...
function renderLogin() {
  ...
  <div class="min-h-screen flex bg-[#FCF8FF] overflow-hidden">
    ...
    <div class="hidden lg:flex flex-col justify-between w-[45%] bg-gradient-to-br from-[#20419A] via-[#2B54C2] to-[#183686] p-14 relative overflow-hidden">
    ...
    <div class="flex-1 flex items-center justify-center p-8">
      <div class="w-full max-w-sm fade-up">
        ...
        <div class="space-y-3 mb-6">
          <button onclick="loginAs('faculty')" class="w-full py-4 rounded-2xl bg-gradient-to-br from-[#20419A] to-[#2B54C2] text-white font-black text-base shadow-lg hover:brightness-110 active:scale-98 transition-all flex items-center justify-center gap-2">
        ...
        <div class="bg-[#F7F2F9] rounded-2xl p-4">
          ...
          <div class="grid grid-cols-2 gap-2">
            <button onclick="fillDemo('faculty')" class="text-left p-3 bg-white rounded-xl hover:bg-[#e5e1e8] transition-colors">
              <p class="text-xs font-black text-[#20419A]">Faculty</p>
            ...
            <button onclick="fillDemo('admin')" class="text-left p-3 bg-white rounded-xl hover:bg-[#e5e1e8] transition-colors">
              <p class="text-xs font-black text-[#20419A]">Admin</p>
  ...
  <aside class="h-screen w-64 fixed left-0 top-0 bg-[#F7F2F9] flex flex-col p-4 gap-1 z-40">
    ...
    <nav class="flex-1 flex flex-col gap-1">
      ${navItems.map(n=>{
        ...
        return `<button onclick="navigate('${n.page}')" class="nav-item flex items-center gap-3 px-4 py-3 rounded-full font-semibold text-sm w-full text-left ${active?'bg-[#2B54C2] text-white shadow-[0px_4px_12px_rgba(32,65,154,0.25)]':'text-[#49454F] hover:bg-[#E5E1E8]'} transition-all relative">
    ...
    <div class="border-t border-[#E5E1E8] pt-4 mb-2">
      <div class="flex items-center gap-3 px-3 mb-3">
        <div class="w-9 h-9 bg-[#2B54C2] rounded-full flex items-center justify-center">
    ...
    <button onclick="openVenueList()" class="bg-gradient-to-br from-[#20419A] to-[#2B54C2] text-white py-4 px-6 rounded-2xl flex items-center justify-center gap-2 font-black text-sm shadow-lg hover:brightness-110 transition-all">
...
function renderTopBar(title) {
  ...
  return `
  <header class="bg-[#FCF8FF]/90 backdrop-blur-md sticky top-0 z-30 flex justify-between items-center px-8 py-4 shadow-[0px_4px_20px_rgba(32,65,154,0.06)]">
    ...
    <div class="flex items-center gap-4">
      ...
      <button class="relative w-10 h-10 rounded-full bg-[#F7F2F9] hover:bg-[#E5E1E8] flex items-center justify-center transition-colors">
        ...
        <span class="absolute top-1.5 right-1.5 w-2.5 h-2.5 bg-[#2B54C2] rounded-full border-2 border-white"></span>
      </button>
      <div class="flex items-center gap-3 pl-3 border-l border-[#E5E1E8]">
        ...
        <div class="w-10 h-10 bg-gradient-to-br from-[#20419A] to-[#2B54C2] rounded-full flex items-center justify-center">
...
function hallCard(h, idx=0) {
  const img = h.image || '';
  return `
  <div class="hall-card bg-white rounded-2xl overflow-hidden flex-shrink-0 w-56 shadow-[0px_4px_20px_rgba(32,65,154,0.07)] cursor-pointer fade-up" style="animation-delay:${idx*60}ms">
    ${img ? `<div class="h-32 overflow-hidden"><img src="${img}" alt="${h.name}" class="w-full h-full object-cover"/></div>` : `<div class="h-32 bg-gradient-to-br from-[#BCCEFF] to-[#9DB8FF] flex items-center justify-center"><span class="material-symbols-outlined text-[#20419A] text-4xl">meeting_room</span></div>`}
    <div class="p-4">
      <div class="flex items-start justify-between mb-2">
        <span class="text-[10px] font-black uppercase tracking-wider text-[#20419A] bg-[#BCCEFF] px-2.5 py-1 rounded-full">${h.room}</span>
      </div>
      ...
      <div class="flex items-center gap-2 flex-wrap mb-3">
        ${(h.amenities||h.features||[]).slice(0,4).map(f=>`<div class="flex flex-col items-center gap-0.5" title="${f.label}">
          <span class="material-symbols-outlined text-[#20419A]" style="font-size:15px">${f.icon}</span>
        </div>`).join('')}
      </div>
      ${h.available!==false
        ? `<button onclick="openBooking('${h.id}')" class="w-full py-2.5 rounded-full bg-[#F7F2F9] hover:bg-[#2B54C2] hover:text-white text-[#20419A] font-black text-xs transition-all">Book</button>`
...
function audiCard(a) {
  ...
  <div class="audi-card group relative bg-surface-container rounded-3xl overflow-hidden h-72 shadow-lg cursor-pointer" style="animation:fadeUp .5s ease both">
    ${a.image
      ...
      : `<div class="w-full h-full bg-gradient-to-br from-[#20419A] to-[#2B54C2] flex items-center justify-center"><span class="material-symbols-outlined text-white/30" style="font-size:80px">theater_comedy</span></div>`
    }
    <div class="absolute inset-0 bg-gradient-to-t from-black/85 via-black/20 to-transparent p-5 flex flex-col justify-end">
      <p class="text-[10px] font-black uppercase tracking-widest text-[#BCCEFF] mb-1">${a.room}</p>
      ...
      <button class="audi-book-btn mt-3 w-full bg-gradient-to-r from-[#20419A] to-[#2B54C2] text-white py-2.5 rounded-xl font-black text-sm" onclick="openBooking('${a.id}')">Book Venue</button>
    </div>
  </div>`;
}
...
function renderDashboard() {
  ...
  <div class="p-8 space-y-12 fade-up">
    ...
    <div class="grid grid-cols-2 md:grid-cols-4 gap-4">
      ${[
        {label:'Total Requests',val:myBks.length,icon:'bookmark',color:'text-[#20419A]',bg:'bg-[#BCCEFF]'},
        ...
        {label:'This Month',val:myBks.filter(b=>new Date(b.createdAt).getMonth()===new Date().getMonth()).length,icon:'calendar_month',color:'text-[#2B54C2]',bg:'bg-[#BCCEFF]'},
      ].map(s=>`<div class="bg-white rounded-2xl p-4 shadow-[0px_4px_16px_rgba(32,65,154,0.06)]">
    ...
    <section>
      <div class="flex items-center justify-between mb-5">
        ...
        <span class="text-xs font-black uppercase tracking-wider text-[#20419A]">${confHalls.length} Space${confHalls.length!==1?'s':''}</span>
      </div>
    ...
    <section>
      <div class="flex items-center justify-between mb-5">
        ...
        <span class="text-xs font-black uppercase tracking-wider text-[#20419A]">${auditoriums.length} Venue${auditoriums.length!==1?'s':''}</span>
      </div>
    ...
    <section class="bg-[#F7F2F9] rounded-3xl p-8 md:p-10 flex flex-col md:flex-row gap-8 items-center fade-up">
      <div class="flex-1 space-y-5">
        <div class="inline-flex items-center gap-2 bg-[#20419A]/10 text-[#20419A] px-3 py-1 rounded-full text-xs font-black uppercase tracking-widest">
        ...
        <div class="grid grid-cols-4 gap-3">
          ${(mfc.amenities||mfc.features||[]).map(f=>`<div class="flex flex-col items-center p-3 bg-white rounded-2xl shadow-sm">
            <span class="material-symbols-outlined text-[#2B54C2] text-2xl mb-1">${f.icon}</span>
            <span class="text-[9px] font-black text-[#787581] uppercase text-center">${f.label}</span>
          </div>`).join('')}
        </div>
        <div class="flex items-center gap-3">
          <button onclick="openBooking('${mfc.id}')" class="bg-gradient-to-br from-[#20419A] to-[#2B54C2] text-white px-8 py-3.5 rounded-2xl font-black text-sm shadow-lg hover:brightness-110 transition-all flex items-center gap-2">
...
function renderMyBookings(forHistory=false) {
  ...
  <div class="p-8 fade-up">
    ...
    <div class="flex gap-2 mb-6 overflow-x-auto pb-1">
      ${tabs.map(([v,l])=>`<button onclick="bookingFilter='${v}';navigate('my-bookings')" class="flex-shrink-0 px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider transition-all ${bookingFilter===v?'bg-[#2B54C2] text-white shadow-md':'bg-[#F7F2F9] text-[#474550] hover:bg-[#E5E1E8]'}">${l}</button>`).join('')}
    </div>`:''}
    ${bks.length===0?`
    <div class="flex flex-col items-center justify-center py-20 text-center">
      ...
      <button onclick="navigate('dashboard')" class="mt-6 bg-[#2B54C2] text-white px-6 py-2.5 rounded-full font-bold text-sm hover:brightness-110 transition-all">Browse Facilities</button>
    </div>`:`
    <div class="space-y-4">
      ${bks.map(b=>`
      <div class="bg-white rounded-2xl p-5 shadow-[0px_4px_16px_rgba(32,65,154,0.06)] fade-up">
        <div class="flex items-start justify-between gap-4">
          <div class="flex items-start gap-4 flex-1 min-w-0">
            <div class="w-11 h-11 bg-[#F7F2F9] rounded-xl flex items-center justify-center flex-shrink-0">
              <span class="material-symbols-outlined text-[#2B54C2] text-xl">${b.typeName==='Auditorium'?'theater_comedy':b.typeName==='Multi Facility Center'?'domain':b.typeName==='Special Permission'?'key':'meeting_room'}</span>
            </div>
            <div class="min-w-0 flex-1">
              <div class="flex items-center gap-2 flex-wrap">
                <p class="font-black text-[#1c1b20]">${b.facilityName}</p>
                <span class="text-[9px] font-black uppercase tracking-wider text-[#20419A] bg-[#BCCEFF] px-2 py-0.5 rounded-full">${b.room}</span>
              </div>
...
function renderNotifications() {
  ...
  <div class="p-8 fade-up">
    ...
    `<div class="space-y-3">
      ${notifs.map(n=>`<div class="bg-white rounded-2xl p-4 shadow-[0px_4px_16px_rgba(32,65,154,0.05)] flex items-start gap-4">
  ...
  <aside class="h-screen w-64 fixed left-0 top-0 bg-[#1C1B20] flex flex-col p-4 gap-1 z-40">
    ...
    <nav class="flex-1 flex flex-col gap-1">
      ${navItems.map(n=>{
        ...
        return `<button onclick="navigate('${n.page}')" class="nav-item flex items-center gap-3 px-4 py-3 rounded-full font-semibold text-sm w-full text-left ${a?'bg-[#20419A] text-white shadow-lg':'text-white/50 hover:bg-white/10 hover:text-white'} transition-all relative">
          <span class="material-symbols-outlined text-xl">${n.icon}</span>
          <span>${n.label}</span>
    ...
    <div class="border-t border-white/10 pt-4 mb-2">
      <div class="flex items-center gap-3 px-3 mb-3">
        <div class="w-9 h-9 bg-[#20419A] rounded-full flex items-center justify-center">
...
function renderAdminOverview() {
  ...
  <div class="p-8 fade-up">
    <div class="grid grid-cols-2 lg:grid-cols-4 gap-4 mb-8">
      ${[
        {l:'Total Bookings',v:counts.total,i:'bookmark',c:'text-[#20419A]',bg:'bg-[#BCCEFF]'},
      ...
      ].map(s=>`<div class="bg-white rounded-2xl p-5 shadow-[0px_4px_16px_rgba(32,65,154,0.06)]">
        <div class="flex items-center justify-between mb-3">
    ...
    <div class="bg-white rounded-2xl shadow-[0px_4px_16px_rgba(32,65,154,0.06)] overflow-hidden">
      <div class="p-5 border-b border-[#F7F2F9] flex items-center justify-between">
        <h3 class="font-black text-[#1c1b20]">Recent Requests</h3>
        <button onclick="navigate('admin-all')" class="text-xs font-bold text-[#20419A] hover:underline">View All</button>
      </div>
  ...
  return `<div class="divide-y divide-[#F7F2F9]">
    ...
    <div class="p-5 flex items-start gap-4 hover:bg-[#FCF8FF] transition-colors">
      <div class="w-10 h-10 bg-[#F7F2F9] rounded-xl flex items-center justify-center flex-shrink-0">
        <span class="material-symbols-outlined text-[#2B54C2] text-lg">${b.typeName==='Auditorium'?'theater_comedy':b.typeName==='Multi Facility Center'?'domain':b.typeName==='Special Permission'?'key':'meeting_room'}</span>
      </div>
      <div class="flex-1 min-w-0">
        <div class="flex items-start justify-between gap-3">
          <div class="min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              ...
              <span class="text-[9px] font-black text-[#20419A] bg-[#BCCEFF] px-2 py-0.5 rounded-full uppercase tracking-wide">${b.room}</span>
            </div>
            <p class="text-sm text-[#474550] mt-0.5 font-medium truncate">${b.purpose}</p>
  ...
  <div class="p-8 fade-up">
    <div class="flex items-center justify-between mb-6">
      ...
      <div class="flex gap-2">
        ${['all','pending','approved','rejected'].map(f=>`<button onclick="adminFilter='${f}';navigate('admin-all')" class="px-4 py-2 rounded-full text-xs font-black uppercase tracking-wider transition-all ${adminFilter===f?'bg-[#20419A] text-white':'bg-[#F7F2F9] text-[#474550] hover:bg-[#E5E1E8]'}">${f}</button>`).join('')}
      </div>
    </div>
    <div class="bg-white rounded-2xl shadow-[0px_4px_16px_rgba(32,65,154,0.06)] overflow-hidden">
...
function renderAdminFacilities() {
  ...
  <div class="p-8 fade-up">
    <div class="flex items-center justify-between mb-6">
      ...
      <button onclick="openAddFacilityModal()" class="flex items-center gap-2 bg-gradient-to-br from-[#20419A] to-[#2B54C2] text-white px-5 py-2.5 rounded-full font-black text-sm shadow-lg hover:brightness-110 transition-all">
    ...
    <div class="space-y-4">
      ...
      <div class="bg-white rounded-2xl shadow-[0px_4px_16px_rgba(32,65,154,0.06)] overflow-hidden">
        ...
        <div class="p-5 flex items-start gap-4">
          <div class="w-11 h-11 bg-[#F7F2F9] rounded-xl flex items-center justify-center flex-shrink-0">
            <span class="material-symbols-outlined text-[#2B54C2] text-xl">${getFacilityIcon(f)}</span>
          </div>
          <div class="flex-1 min-w-0">
            <div class="flex items-center gap-2 flex-wrap">
              <p class="font-black text-[#1c1b20] text-sm">${f.name}</p>
              <span class="text-[9px] font-black text-[#20419A] bg-[#BCCEFF] px-2 py-0.5 rounded-full uppercase tracking-wide">${f.room}</span>
          ...
          <div class="flex items-center gap-2 flex-shrink-0">
            ...
            <button onclick="openViewSlotsPanel('${f.id}')" class="w-8 h-8 rounded-xl bg-[#BCCEFF] hover:bg-[#2B54C2] hover:text-white text-[#20419A] flex items-center justify-center transition-all" title="View / Select Slots">
              <span class="material-symbols-outlined text-sm">calendar_view_week</span>
            </button>
            <button onclick="openEditFacilityModal('${f.id}')" class="w-8 h-8 rounded-xl bg-[#F7F2F9] hover:bg-[#20419A] hover:text-white text-[#474550] flex items-center justify-center transition-all" title="Edit">
  ...
  <div class="p-5 bg-[#F7F2F9] rounded-b-2xl">
    <div class="flex items-center justify-between mb-4 flex-wrap gap-3">
      ...
      <div class="flex items-center gap-2">
        ...
        <button onclick="confirmAdminSlotBlock('${f.id}')" class="px-4 py-2 rounded-xl bg-[#20419A] text-white font-black text-xs hover:brightness-110 transition-all flex items-center gap-1">
    ...
    <div id="slot-selection-summary-${f.id}" class="mt-3 text-xs font-semibold text-[#20419A]"></div>
  </div>`;
}
    ...
    <thead><tr>
      ...
      ${weekDates.map((d,i)=>{
        ...
        return `<th style="text-align:center;padding:4px 2px;min-width:68px">
          <div style="font-size:10px;font-weight:800;color:${isSelected?'#20419A':'#787581'};text-transform:uppercase">${DAY_LABELS[i]}</div>
          <div style="font-size:11px;font-weight:900;color:${isSelected?'#20419A':'#1c1b20'};background:${isSelected?'#BCCEFF':'transparent'};border-radius:6px;padding:2px 4px">
...
function openAddFacilityModal(existingId) {
  ...
  <div class="fixed inset-0 bg-[#1C1B20]/70 backdrop-blur-sm flex items-center justify-center p-4 fade-in" onclick="handleOverlayClick(event)">
    <div class="bg-[#FCF8FF] rounded-3xl shadow-2xl w-full max-w-2xl scale-in overflow-hidden flex flex-col" style="max-height:92vh" onclick="event.stopPropagation()">
      <div class="bg-gradient-to-br from-[#20419A] to-[#2B54C2] p-6 text-white flex-shrink-0">
        <div class="flex justify-between items-center">
          <div>
      ...
      <div class="p-6 overflow-y-auto space-y-5">
        ...
        <div>
          ...
          <div class="flex gap-2 mb-3">
            <button type="button" id="img-tab-url" onclick="switchImgTab('url')" class="px-4 py-2 rounded-full text-xs font-black transition-all bg-[#20419A] text-white">Image URL</button>
          ...
          <div id="img-panel-upload" class="hidden">
            <div class="border-2 border-dashed border-[#c9c4d2] rounded-2xl p-6 text-center bg-white hover:border-[#2B54C2] transition-colors cursor-pointer" onclick="document.getElementById('af-image-file').click()">
        ...
        <div class="flex items-center gap-3">
          ...
          <label class="flex items-center gap-2 cursor-pointer">
            <input id="af-available" type="checkbox" ${existing?.available===false?'':'checked'} class="w-4 h-4 rounded accent-[#20419A]"/>
        ...
        <div>
          ...
          <div class="grid grid-cols-4 gap-2" id="amenity-grid">
            ...
            <button type="button" id="am-${a.id}" onclick="toggleAmenity('${a.id}')"
              class="flex flex-col items-center gap-1 p-3 rounded-xl border-2 transition-all text-center amenity-btn ${selAmenities.has(a.id)?'border-[#2B54C2] bg-[#BCCEFF] text-[#20419A]':'border-[#e5e1e8] bg-white text-[#787581] hover:border-[#c9c4d2]'}">
      ...
      <div class="p-6 pt-4 flex gap-3 flex-shrink-0 border-t border-[#E5E1E8]">
        <button onclick="closeModal()" class="flex-1 py-3 rounded-full bg-[#E5E1E8] text-[#474550] font-bold text-sm hover:bg-[#c9c4d2] transition-colors">Cancel</button>
        <button onclick="saveFacility(${isEdit?`'${existingId}'`:'null'})" class="flex-1 py-3 rounded-full bg-gradient-to-br from-[#20419A] to-[#2B54C2] text-white font-black text-sm shadow-lg hover:brightness-110 transition-all flex items-center justify-center gap-2">
...
function switchImgTab(tab) {
  ...
  if(tab==='url') {
    ...
    urlBtn.className='px-4 py-2 rounded-full text-xs font-black transition-all bg-[#20419A] text-white';
    ...
    uploadBtn.className='px-4 py-2 rounded-full text-xs font-black transition-all bg-[#20419A] text-white';
...
function toggleAmenity(id) {
  ...
  if(window._editAmenities.has(id)) {
    ...
    btn.classList.remove('border-[#2B54C2]','bg-[#BCCEFF]','text-[#20419A]');
    btn.classList.add('border-[#e5e1e8]','bg-white','text-[#787581]','hover:border-[#c9c4d2]');
  } else {
    window._editAmenities.add(id);
    btn.classList.add('border-[#2B54C2]','bg-[#BCCEFF]','text-[#20419A]');
